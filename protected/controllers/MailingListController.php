...
class MailingListController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','emailer'),
				'users'=>array('*'),
			),
	...
	{
		$this->render('//mail/july_14_news',array('model'=>$this->loadModel($id)));
	}
	...
	public function actionEmailer()
		{
			...
			foreach ($emails as $row)
			{
				...
				if(isset($_POST['MailingList']))
				{
						...
						$message->view = 'july_14_news';
						$message->setBody(array('model'=>$row), 'text/html');
						$message->subject = 'Jakata July News';
			...
			$this->render('emailer',array('model'=>$model));
		}
	...
	{
		$dataProvider=new CActiveDataProvider('MailingList');
	...
	{
		$model=new MailingList('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MailingList']))
			$model->attributes=$_GET['MailingList'];
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='mailing-list-form')
		{
